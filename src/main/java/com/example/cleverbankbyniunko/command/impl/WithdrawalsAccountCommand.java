...
public class WithdrawalsAccountCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException, DaoException {
        ...
        BigDecimal value=new BigDecimal(request.getParameter(AttributeName.WITHDRAWALS_AMOUNT)).setScale(2, RoundingMode.DOWN);
        ...
        try{
            if (transactionService.withdrawalsAccount(value.doubleValue(), account)){
                ...
                if (optionalAccount.isPresent() && accountService.findAllAccountsByUserID(user.getId(),accountList)){
                    ...
                    session.setAttribute(AttributeName.MESSAGE_USER_PAGE, PageMessages.WITHDRAWALS_SUCCESSFUL_COMPLETE_MESSAGE);
                }
                ...
                session.setAttribute(AttributeName.MESSAGE_USER_PAGE,PageMessages.WITHDRAWALS_NOT_SUCCESSFUL_COMPLETE_MESSAGE);
                router.setPage(page);
            }
        }catch (ServiceException e){
            logger.warn("Failed to withdrawals accoount in withdrawalsCommand");
        }
        return router;
    }
}
